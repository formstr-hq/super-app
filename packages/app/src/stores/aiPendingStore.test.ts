...
describe("moduleForTool", () => {
  ...
  it("returns null for tools the app no longer surfaces", () => {
    // Pages and Polls still exist in the agent registry for the MCP server,
    // but this app has no UI for them and never dispatches them.
    expect(moduleForTool("create_page")).toBeNull();
    expect(moduleForTool("create_poll")).toBeNull();
  });
});
