...
>   [super-app/upstream/nostr-calendar/](super-app/upstream/nostr-calendar/).
>   Ships formal NIP proposals in
>   [super-app/upstream/nostr-calendar/nips/](super-app/upstream/nostr-calendar/nips/).
> - **`@formstr/agent` calendar service** — a headless TS implementation already
>   factored out of any UI, under
>   [super-app/packages/agent/src/services/calendar/](super-app/packages/agent/src/services/calendar/).
...
14. [Suggested package layout](#14-suggested-package-layout)
...
([super-app/packages/agent/src/services/calendar/service.ts](super-app/packages/agent/src/services/calendar/service.ts)).
...
[super-app/packages/core/src/signer/types.ts](super-app/packages/core/src/signer/types.ts).
...
> instance: `{ nip44Encrypt: (pk, pt) => signer.nip44Encrypt(pk, pt) }`.

### 3.2 Relay runtime

A thin relay-pool facade. Source:
[super-app/packages/core/src/runtime/NostrRuntime.ts](super-app/packages/core/src/runtime/NostrRuntime.ts).
...
[super-app/packages/core/src/relay/module-defaults.ts](super-app/packages/core/src/relay/module-defaults.ts).
...
([super-app/upstream/nostr-calendar/src/common/EventConfigs.ts](super-app/upstream/nostr-calendar/src/common/EventConfigs.ts))
and the agent `CALENDAR_KINDS`
([super-app/packages/agent/src/services/calendar/types.ts](super-app/packages/agent/src/services/calendar/types.ts)).

| Kind          | Name                          | Class                  | Encrypted?           | Authored by | Purpose                             |
| ------------- | ----------------------------- | ---------------------- | -------------------- | ----------- | ----------------------------------- |
| **31923**     | Public time event             | Param-replaceable      | No                   | user        | NIP-52 public event                 |
| **31922**     | Public day event              | Param-replaceable      | No                   | user        | NIP-52 all-day event (date strings) |
| **32678**     | Private time event            | Param-replaceable      | **NIP-44 (viewKey)** | user        | Encrypted time event                |
| **32681**     | Private day event             | Param-replaceable      | **NIP-44 (viewKey)** | user        | Encrypted all-day event (NIP-52E)   |
...
- **Day events (31922 / 32681)** carry `start`/`end` as `YYYY-MM-DD` **date
  strings**, with an **exclusive** `end` (NIP-52). Time events (31923 / 32678) use
  unix-second integers. The agent currently models everything as time events; a
  full SDK should support both.
...
[super-app/packages/core/src/crypto/nip44.ts](super-app/packages/core/src/crypto/nip44.ts),
[super-app/packages/core/src/crypto/nip59.ts](super-app/packages/core/src/crypto/nip59.ts).
...
key. Reference:
[super-app/packages/agent/src/services/calendar/viewKey.ts](super-app/packages/agent/src/services/calendar/viewKey.ts).
...
Reference: [super-app/packages/core/src/crypto/nip59.ts](super-app/packages/core/src/crypto/nip59.ts).
...
Timestamps on the seal and wrap are randomized **±2 days** to defeat timing
correlation. The wrap's signer is a throwaway key, so relays can't see who sent it.
...
> Public events are authored _by the user_ and discoverable _by author_ — but note
> **`calendar.formstr.app` does not render foreign public events in its main view**
> and public refs can't be privately listed, so cross-app sync of public events is
> effectively one-way. Default to private events ([§13.1](#131-discovery-the-1-bug)).
...
([super-app/packages/agent/src/services/calendar/service.ts](super-app/packages/agent/src/services/calendar/service.ts)),
and NIP-52E
([super-app/upstream/nostr-calendar/nips/NIP-52E.md](super-app/upstream/nostr-calendar/nips/NIP-52E.md)).
...
spec: [super-app/upstream/nostr-calendar/RSVP_SPEC.md](super-app/upstream/nostr-calendar/RSVP_SPEC.md).
Parsing maps it to `registrationFormRef` / `registrationFormViewKey`
([service.ts `parseCalendarEvent`](super-app/packages/agent/src/services/calendar/service.ts)).
...
codec at
[super-app/packages/agent/src/services/calendar/calendarListCodec.ts](super-app/packages/agent/src/services/calendar/calendarListCodec.ts).
...
`d`-tag derivation (upstream): `SHA-256("<title>:<created_at>").slice(0,16)`; the
agent uses `crypto.randomUUID().slice(0,8)`. Either is fine — it only needs to be
unique per author.

**Decode quirks your parser must tolerate** (all from real on-relay data — see
[`decodeCalendarList`](super-app/packages/agent/src/services/calendar/calendarListCodec.ts)):
...
[super-app/packages/agent/src/services/calendar/rsvp.ts](super-app/packages/agent/src/services/calendar/rsvp.ts)).
...
Reference: [super-app/packages/agent/src/services/calendar/rsvp.ts](super-app/packages/agent/src/services/calendar/rsvp.ts)
and [super-app/upstream/nostr-calendar/RSVP_SPEC.md](super-app/upstream/nostr-calendar/RSVP_SPEC.md).
...
([service.ts](super-app/packages/agent/src/services/calendar/service.ts)).
...
[super-app/packages/agent/src/services/calendar/busyList.ts](super-app/packages/agent/src/services/calendar/busyList.ts),
NIP-52E §5.
...
[super-app/upstream/nostr-calendar/SCHEDULING_PROTOCOL.md](super-app/upstream/nostr-calendar/SCHEDULING_PROTOCOL.md)
and the NIP at
[super-app/upstream/nostr-calendar/nips/NIP-Appointment-Scheduling.md](super-app/upstream/nostr-calendar/nips/NIP-Appointment-Scheduling.md).
Read/approve reference:
[super-app/packages/agent/src/services/calendar/booking.ts](super-app/packages/agent/src/services/calendar/booking.ts).
...
> **Timestamp note.** The upstream spec wraps booking req/resp with a **real**
> timestamp (`useRealTimestamp`) rather than the ±2-day jitter. The core
> `wrapEvent` used by the agent **always** jitters
> ([nip59.ts](super-app/packages/core/src/crypto/nip59.ts)), so the agent's booking
> wraps are jittered too — harmless here because matching ignores the wrap time. If
> you want strict upstream parity, make the ±2-day randomization optional in your
> `wrapEvent` and pass real time for booking wraps.
...
Spec: [super-app/upstream/nostr-calendar/nips/NIP-52R.md](super-app/upstream/nostr-calendar/nips/NIP-52R.md).
Helper: [super-app/upstream/nostr-calendar/src/utils/repeatingEventsHelper.ts](super-app/upstream/nostr-calendar/src/utils/repeatingEventsHelper.ts).
...
`calendar.formstr.app`'s main view renders **only channel 2** — events referenced
(with a viewKey) in a _visible_ calendar list. It does **not** query events by
author and its public-event fetcher is dead code. **Consequence:** an event that is
never _listed_ is invisible there. This is the origin of the "created via API but
doesn't show on the website" bug ([§13.1](#131-discovery-the-1-bug)).
...
([super-app/upstream/nostr-calendar/src/common/nostr.ts](super-app/upstream/nostr-calendar/src/common/nostr.ts)).
...
([super-app/packages/agent/src/tools/calendar.ts](super-app/packages/agent/src/tools/calendar.ts))
...
[super-app/upstream/nostr-calendar/src/utils/types.ts](super-app/upstream/nostr-calendar/src/utils/types.ts).
...
[service.ts `createCalendarEvent`](super-app/packages/agent/src/services/calendar/service.ts).
...
[booking.ts `approveBookingRequest`](super-app/packages/agent/src/services/calendar/booking.ts).
...
[super-app/upstream/nostr-calendar/src/common/calendarEngine.ts](super-app/upstream/nostr-calendar/src/common/calendarEngine.ts);
orchestrator:
...
build wrote an object; deployed `calendar.formstr.app` builds **throw** on it and
that throw aborts their _entire_ calendar-list load — making _none_ of the user's
events appear. `fetchCalendarLists` detects an object payload, loads it, _and_
re-publishes it as the array shape to heal the relay copy (self-limiting).
...
@calendar-sdk/
  src/
    contracts.ts        # NostrSigner, NostrRuntime, RelayProvider interfaces (§3)
    kinds.ts            # CALENDAR_KINDS (§4)
    crypto/
      nip44.ts          # self-encrypt/decrypt wrappers (§5.2)
      nip59.ts          # wrapEvent / unwrapEvent (§5.4)
...
- [super-app/upstream/nostr-calendar/PROTOCOL.md](super-app/upstream/nostr-calendar/PROTOCOL.md) — private events, lists, gift wraps, fetch strategy, caching
- [super-app/upstream/nostr-calendar/nips/NIP-52E.md](super-app/upstream/nostr-calendar/nips/NIP-52E.md) — 32678/32681/1052/32123/31926/84 formal spec
- [super-app/upstream/nostr-calendar/nips/NIP-52R.md](super-app/upstream/nostr-calendar/nips/NIP-52R.md) — recurrence via RRULE labels
- [super-app/upstream/nostr-calendar/RSVP_SPEC.md](super-app/upstream/nostr-calendar/RSVP_SPEC.md) — form-backed RSVP, `["form", naddr, viewKey]`
- [super-app/upstream/nostr-calendar/SCHEDULING_PROTOCOL.md](super-app/upstream/nostr-calendar/SCHEDULING_PROTOCOL.md) — 31927/32680/1057/1058 booking
- [super-app/upstream/nostr-calendar/nips/NIP-Appointment-Scheduling.md](super-app/upstream/nostr-calendar/nips/NIP-Appointment-Scheduling.md)

**Headless reference implementation (model the SDK on this):**

- [super-app/packages/agent/src/services/calendar/service.ts](super-app/packages/agent/src/services/calendar/service.ts) — events, lists, discovery, deletions
- [super-app/packages/agent/src/services/calendar/viewKey.ts](super-app/packages/agent/src/services/calendar/viewKey.ts) — view-key crypto
- [super-app/packages/agent/src/services/calendar/calendarListCodec.ts](super-app/packages/agent/src/services/calendar/calendarListCodec.ts) — list encode/decode + heals
- [super-app/packages/agent/src/services/calendar/rsvp.ts](super-app/packages/agent/src/services/calendar/rsvp.ts) — RSVP + invitation extract
- [super-app/packages/agent/src/services/calendar/busyList.ts](super-app/packages/agent/src/services/calendar/busyList.ts) — busy lists
- [super-app/packages/agent/src/services/calendar/booking.ts](super-app/packages/agent/src/services/calendar/booking.ts) — scheduling pages + approve/decline
- [super-app/packages/agent/src/services/calendar/types.ts](super-app/packages/agent/src/services/calendar/types.ts) — kinds + domain types
- [super-app/packages/agent/src/tools/calendar.ts](super-app/packages/agent/src/tools/calendar.ts) — verb/argument surface

**Infrastructure contracts:**

- [super-app/packages/core/src/signer/types.ts](super-app/packages/core/src/signer/types.ts) — `NostrSigner`
- [super-app/packages/core/src/runtime/NostrRuntime.ts](super-app/packages/core/src/runtime/NostrRuntime.ts) — `querySync` / `subscribe` / `publish`
- [super-app/packages/core/src/crypto/nip44.ts](super-app/packages/core/src/crypto/nip44.ts), [super-app/packages/core/src/crypto/nip59.ts](super-app/packages/core/src/crypto/nip59.ts)
- [super-app/packages/core/src/relay/module-defaults.ts](super-app/packages/core/src/relay/module-defaults.ts) — relay defaults

**UI reference:**

- [super-app/upstream/nostr-calendar/src/common/calendarEngine.ts](super-app/upstream/nostr-calendar/src/common/calendarEngine.ts) — day segments + layout
- [super-app/upstream/nostr-calendar/src/utils/repeatingEventsHelper.ts](super-app/upstream/nostr-calendar/src/utils/repeatingEventsHelper.ts) — RRULE expansion
